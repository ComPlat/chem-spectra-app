...
def test_datatable_encode_empty():
    ...
    calculate_ys = data_table_model.encode(arr_ys=np.array([]), y_factor=None, arr_xs=None, is_xypoints=False)
    assert calculate_ys == []
...
def test_datatable_encode_xypoints():
    ...
    calculate_ys = data_table_model.encode(arr_ys=arr_ys, y_factor=None, arr_xs=arr_xs, is_xypoints=True)
    assert calculate_ys == [
        '1.49048, 5.34724\n',
        '1.48049, 4.81545\n']
